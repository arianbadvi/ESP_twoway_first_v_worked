...
void MLX90614_WriteReg(uint8_t devAddr, uint8_t regAddr, uint16_t data) {
	...
	HAL_I2C_Master_Transmit(_mlx_i2c_port, (devAddr << 1), i2cdata, 4, 0xFFFF);
	HAL_Delay(10);
...
uint16_t MLX90614_ReadReg(uint8_t devAddr, uint8_t regAddr, uint8_t dbg_lvl) {
	...
	HAL_I2C_Mem_Read(_mlx_i2c_port, (devAddr<<1), regAddr, 1, in_buff, 3, 100);
...
void MLX90614_ScanDevices (void) {
	...
	for (int i = 0; i<128; i++)
		  {
			  result = HAL_I2C_IsDeviceReady(_mlx_i2c_port, (uint16_t) (i<<1), 2, 2);
			  if (result != HAL_OK)
			  {
				  sprintf(temp_buff, ".");
